...
use super::config_types::DEFAULT_USE_TABS;
...
pub fn resolve_config(
  ...
  let use_tabs_default = global_config.use_tabs.unwrap_or(DEFAULT_USE_TABS);
  ...
  let resolved_config = Configuration {
    line_width: get_u32(&mut config, "lineWidth", line_width_default, &mut diagnostics),
    use_tabs: get_bool(&mut config, "useTabs", use_tabs_default, &mut diagnostics),
    indent_width: get_u8(&mut config, "indentWidth", indent_width_default, &mut diagnostics),
    new_line_kind: parse_new_line_kind(&mut config, "newLineKind", &new_line_kind_default, &mut diagnostics),
  };
...
pub fn resolve_global_config(config: &mut ConfigKeyMap) -> ResolveConfigurationResult<GlobalConfiguration> {
  ...
  let resolved_config = GlobalConfiguration {
    line_width: Some(get_u32(&mut config, "lineWidth", DEFAULT_LINE_WIDTH, &mut diagnostics)),
    use_tabs: Some(get_bool(&mut config, "useTabs", DEFAULT_USE_TABS, &mut diagnostics)),
    ...
    new_line_kind: Some(get_string(
      &mut config,
      "newLineKind",
      DEFAULT_NEW_LINE_KIND,
      &mut diagnostics,
    )),
  };
...
fn get_unknown_property_diagnostics(config: ConfigKeyMap) -> Vec<ConfigurationDiagnostic> {
  config.into_keys().map(|key| ConfigurationDiagnostic {
      property: key,
    ...
    Some(ConfigKeyValue::Text(text)) => match text.parse::<bool>() {
      ...
      Err(_) => {
        diagnostics.push(ConfigurationDiagnostic {
          ...
          message: format!("Expected a boolean for '{}'.", name),
        });
        default_value
      }
    },
...
fn get_string(
  ...
  name: &str,
  default_value: &str,
  _diagnostics: &mut Vec<ConfigurationDiagnostic>,
) -> String {
  match config.remove(name) {
...
fn parse_new_line_kind(
  ...
  let raw_value = get_string(config, name, default_value, diagnostics);
